...
min_th_A = -142.8*deg; % Minimum /Attainable/ Angle for Joint A in Freespace (no obstacles)
max_th_A = 142.8*deg; % Max
min_th_B = -150.45*deg; % Minimum /Attainable/ Angle for Joint B in Freespace (no obstacles)
max_th_B = 150.45*deg; % Max
step_th = ( (360/200) ) * deg; % Minimum meaningful (or attainable) change in th.
step_th = step_th / 4.4615384615 / 15; % Gear Reduction
step_th = step_th / 2; % Microstepping
step_th = step_th * 4; % Scaling Factor (to save memory).

len_AB = 162.5; % Distance from Joint A to Joint B
len_BF = 162.5; % Distance from Joint B to End Effector

width_arm = 80.82; % Width of Arm Segment (Arm Segment is Slot Shaped)
...
box_width_cells = floor(box_width_cells/2)*2 + 1; % Round Up to Nearest Odd Number
                                                                           ws_mem_size = (box_width_cells*box_width_cells*8 + 100)/(1e6);
                                                                           fprintf("Workspace Matrix Size: %dx%d, %0.5fMB\n", box_width_cells,box_width_cells, ws_mem_size);
...
function drawRect(x1,y1,x2,y2)
    ...
    WS_XY( XY_mid+ceil(ya/res_xy):XY_mid+ceil(yb/res_xy), XY_mid+ceil(xa/res_xy):XY_mid+ceil(xb/res_xy) ) = 1;
                                                                           ...
                                                                           disp("Configuring Workspace. . . " + num2str(toc));
                                                                           pause(0.1);

drawRect(-120.59, 286.59, 120.59, 45.41);

% Add Boundaries:
% No Solid (constraining) Bounds
...
buff_cells = ceil(buff/res_xy)+0.5; % Convert Buffer to Cells and Ensure Twice the value is Odd (width of mask)
                                                                           ...
                                                                           disp("Buffering Configuration Space. . . " + num2str(toc));
                                                                           pause(0.1);
...
config_AB( th2idx(max_th_A:step_th:pi), : ) = 1;
                                                                           ...
                                                                           disp("Computing Configuration Accessibility. . . " + num2str(toc));
                                                                           fprintf(">\t %0.1f%%\n", 0);
                                                                           last_progress = 0.0; % Fraction of Job Done
...
thA = ths_A(a);
    ...
    thB = ths_B(b);
        ...
        access_XY(i,j) = acc & access_XY(i,j); % Make sure not to override accessibility status if location already determined accessible by another configuration 
    b = b+1;
    end % thB
a = a+1;

                                                                           % Update Progress:
                                                                           if((thA-min_th_A )/(max_th_A-min_th_A) >= (progress_update_interval + last_progress))
                                                                               progress_status = 100*(thA-min_th_A)/(max_th_A-min_th_A);
                                                                               fprintf(">\t %0.1f%%\n", progress_status);
                                                                            last_progress = last_progress + progress_update_interval;
                                                                           end % progress update
...
%% Display:
if(do_plot)
                                                                           disp("Displaying Results. . . " + num2str(toc));
    figure();
    imagesc(WS_XY);
    title({'XY Workspace', '(no arm constraints)'});
    % Generate Tick Marks:
    dist_spacing = 4*25.4; % Tick Increments of 4 Inches
    ...
    toc
                                                                           ...
                                                                           disp("Configuration Space Planning Complete! ");
