...
#include <Motion.h>
  ...
  #define PRIMARY_USART Serial1 // Primary Full-Duplex USART Comms Pair: TTX and RRX
  #define SECONDARY_USART Serial3 // Secondary Full-Duplex USART Comms Pair: RTX and TRX



  void init_comm(){
    // Establish initial values for RS485 translator settings before enabling output:
    // (all inactive by default)
    digitalWrite(TDE, LOW);
    digitalWrite(_RRE, HIGH);
    digitalWrite(RDE, LOW);
    digitalWrite(_TRE, HIGH);
    pinMode(TDE, OUTPUT);
    pinMode(_RRE, OUTPUT);
    pinMode(RDE, OUTPUT);
    pinMode(_TRE, OUTPUT);
