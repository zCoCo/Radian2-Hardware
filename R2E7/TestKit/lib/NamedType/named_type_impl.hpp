...
template<typename T>
using IsNotReference = typename etl::enable_if<!etl::is_reference<T>::value, void>::type;
...
public:
    ...
    explicit constexpr NamedType(T&& value) : value_(etl::move(value)) {}

    // get
    constexpr T& get() { return value_; }
    constexpr etl::remove_reference_t<T> const& get() const {return value_; }
    ...
    struct argument
    {
        ...
        {
            return NamedType(etl::forward<U>(value));
        }
        argument() = default;
