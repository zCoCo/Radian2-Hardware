...
protected:
    ...
    int step_count;        // current position
    int steps_remaining;   // to complete the current move (absolute value)
    int steps_to_cruise;   // steps to reach cruising (max) rpm
    int steps_to_brake;    // steps needed to come to a full stop
    int step_pulse;        // step pulse duration (microseconds)
...
public:
     ...
     */
    long getTimeForMove(int steps);
    int calcStepsForRotation(float deg){
